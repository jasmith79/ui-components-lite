...
`;export default defineUIComponent({name:'ui-drop-down',reflectedAttributes,template,definition:class extends mix(UIBase).with(ListBehavior,Focusable){constructor(){super(),this._list=null,this._listHolder=null,this._dummyItem=null,this._textContent=''}get textContent(){return this._dummyItem&&this._dummyItem.textContent||this._textContent}set textContent(a){const b=a||'';return this._textContent=b,this._dummyItem||(this._dummyItem=this.selectInternalElement('#dummy-item')),this._dummyItem.querySelector('#dummy-item-content').textContent=b,'...'===b?this._dummyItem.classList.add('default'):this._dummyItem.classList.remove('default'),this}appendChild(a){return a&&(super.appendChild(a),a.on('click',()=>{this.multiple||global.setTimeout(()=>{this.close()},300)})),a}toggle(){return this.isOpen=!this.isOpen,this}open(){return this.isOpen=!0,this}close(){return this.isOpen=!1,this}init(){let a=!1;super.init();const b=this.attr('tabindex');(null===b||0>b)&&this.attr('tabindex','0'),this.on('enter-key',()=>{this.open()}),this.attr('name')&&(!this.attr('label')&&this.attr('label',this.attr('name')),this.selectInternalElement('label').setAttribute('for',this.attr('name'))),this.attr('label')&&this.selectInternalElement('label').classList.add('text-moved'),this.on('focus',()=>this.selectInternalElement('label').classList.remove('text-moved')),this.on('blur',()=>{global.setTimeout(()=>{this.label&&!this.value&&this.selectInternalElement('label').classList.add('text-moved')},600)}),this._beforeReady(()=>{this._list=this.selectInternalElement('ui-list'),this._listHolder=this.selectInternalElement('#list-holder'),this._dummyItem=this.selectInternalElement('#dummy-item'),this._dummyItem.selectInternalElement('ui-checkbox').style.display='none',this._items.forEach((a)=>{a.isSelected&&(this.selected=a),a.on('click',()=>{this.multiple||global.setTimeout(()=>{this.close()},300)})}),this.name&&!this.selected&&(this.textContent=null),this._listHolder.classList.remove('not-overflowing'),this._dummyItem.on('click',()=>{null===this.attr('tabindex')&&this.attr('tabindex','0'),this.toggle(),a=this.isOpen})}),this.multiple||(this.multiple=!1),this.isOpen||(this.isOpen=!1),this.on('mouseenter',()=>a=!0),this.on('mouseleave',()=>{a=!1,global.setTimeout(()=>{a||(this.isOpen=!1)},1e3)}),this.on('attribute-change',({changed:{now:a,name:b}})=>{'selected-index'===b?this.textContent=this.selected&&!this.multiple?this.selected.textContent:'':void 0})}}});